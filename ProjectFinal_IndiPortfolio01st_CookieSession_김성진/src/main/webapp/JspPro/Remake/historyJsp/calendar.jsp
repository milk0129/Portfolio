    ...
    pageEncoding="UTF-8"%>
		...
		function clickDate(self, stDay) {
		    ...
		    for (let j = 0; j < splDbAry.length; j++) {
		        ...
		        if (parseInt(dbYear) === parseInt(curYear) && parseInt(dbMonth) === parseInt(curMonth) && parseInt(dbDay) === parseInt(onlyDate)) {
		            hasData = true;
		    ...
		    if (!hasData) {
		        ...
		        const addRecordURL = '<%= request.getContextPath() %>/JspPro/Remake/mainJsp/login_index.jsp?category=user&menu=add&selectedDate=' + selectedDateValue;
...
<div id = "calendarWrapper">
        <form id="calendarFrm" name="calendarFrm" method="get" action="<%= request.getContextPath() %>/JspPro/Remake/mainJsp/login_index.jsp">
		    <!--  선택한 날짜값 -->
